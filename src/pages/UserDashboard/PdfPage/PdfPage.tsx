 ...
 * Shows PDF folders (type=PDF) with the same grid/list UI as the bookmark dashboard.
...
export const PdfPage: React.FC = () => {
  ...
  const fetchFolders = async (showSuccessFeedback = false) => {
    ...
    try {
      setIsLoading(true);
      const response = await getAllFolders(accessToken, 'PDF');
  ...
  const handleRefresh = () => fetchFolders(true);
  ...
  const handleCreateFolder = async (name: string) => {
    ...
    try {
      await createFolder(accessToken, name, undefined, 'PDF');
      await fetchFolders();
  ...
  const handleDeleteClick = (folderId: string) => setDeleteConfirmFolderId(folderId);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const removeFolderById = (folderList: FolderWithSubFolders[], id: string): FolderWithSubFolders[] =>
        folderList
          .filter(f => f.id !== id)
          .map(f => ({ ...f, subFolders: removeFolderById(f.subFolders || [], id) }));

      setFolders(prev => removeFolderById(prev, deleteConfirmFolderId));
    ...
    try {
      ...
      const updateFolderName = (folderList: FolderWithSubFolders[], id: string, newName: string): FolderWithSubFolders[] =>
        folderList.map(f => {
          ...
          if (f.subFolders && f.subFolders.length > 0) return { ...f, subFolders: updateFolderName(f.subFolders, id, newName) };
          return f;
        });

      setFolders(prev => updateFolderName(prev, folderId, updatedFolder.name));
  ...
  return (
    <div className={styles.container}>
      <h2 className={styles.heading}>My PDFs</h2>
      ...
      {isLoading && folders.length === 0 ? (
        ...
        <div className={styles.emptyState}>
          ...
          <p className={styles.emptyMessage}>Create a folder to organise your PDFs.</p>
        </div>
      ) : (
        <>
          {viewMode === 'list' ? (
            <div className={styles.listViewWrapper}>
              <div className={styles.listView}>
                <DataTable
                  ...
                  onRowHover={(folder) => {
                    if (folder) setHoveredRowId(folder.id);
      ...
      {deleteConfirmFolderId && (
        <ConfirmDialog
          ...
          message="All PDFs inside this folder will be removed from the folder. Are you sure?"
